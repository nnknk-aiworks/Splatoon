...
internal class P2_Missing_1238_4567_Fix_Partner_Mixed : SplatoonScript
{
    ...
    public override Metadata? Metadata => new(1, "nnknk");
    ...
    // The non-chosen candidate slot of a duplicate call-out (highlighted, not tethered).
    private const string ElMyRoleAlt = "MyRoleAlt";
    // Live marker placed on the same-debuff teammate of a duplicate call-out.
    private const string ElPartnerMark = "PartnerMark";

    // Dimmed semi-transparent gray for other players' stations (gray is byte-order agnostic).
    private const uint OtherRoleColor = 0xA0AAAAAA;
    // Bright emphasis for the player's two duplicate-call-out candidate slots / teammate marker.
    private static readonly uint CandidateAltColor = EColor.YellowBright.ToUint();
    private static readonly uint PartnerMarkColor = EColor.VioletBright.ToUint();
    // Below this snapshot bearing separation the two duplicate players are treated as ambiguous.
    private const float SnapshotMinSeparationDeg = 2f;
    ...
    private sealed class PlayerInfo
    {
        ...
        // Compass bearing from arena center captured when the debuff first appeared (frozen, not live).
        public float? SnapshotBearing;
        // The debuff kind the current snapshot was captured for (used for rising-edge detection).
        public DebuffKind SnapshotBearingDebuff;
    }
    ...
    public override void OnSetup()
    {
        ...
        // The other candidate slot of a duplicate call-out: highlighted but never tethered.
        ...
        // Live ring drawn on the same-debuff teammate so the player can identify them.
        Controller.RegisterElement(ElPartnerMark, new Element(0)
        {
            Enabled = false,
            radius = 0.6f,
            Donut = 0.15f,
    ...
    private void DrawPrioritySettings()
    {
        ...
        ImGui.Spacing();
        ImGui.Checkbox("Show all role positions in combat (your own tethered, others shown dimmed)",
            ref C.ShowAllRoles);
    ...
    private void DrawPlayerInfosTable(string title, IReadOnlyList<PlayerInfo> infos)
    {
        ...
        if(!ImGui.BeginTable($"##12384567PairInfos{title}", 7,
               ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.SizingStretchProp))
            return;
        ...
        ImGui.TableSetupColumn("Side", ImGuiTableColumnFlags.WidthFixed, 60f);
        ImGui.TableSetupColumn("Snap", ImGuiTableColumnFlags.WidthStretch);
        ImGui.TableHeadersRow();
        ...
        for(var i = 0; i < infos.Count; i++)
        {
            ...
            ImGui.TableNextColumn();
            ImGui.TextUnformatted(info.SnapshotBearing is { } bearing ? $"{bearing:F0}°" : "—");
    ...
    private void UpdateFieldMarkers()
    {
        ...
        // Detect a duplicate call-out: my role is one of a left/right pair and a same-debuff tower
        // partner exists. Then both candidate slots are highlighted and the partner is marked live,
        // but only my resolved slot is tethered.
        ...
        var altLabel = "";
        if(baseInfo != null
            && TryGetDualCandidateRoleLabels(roleLabel, out var leftLabel, out var rightLabel))
        {
            partner = FindSameDebuffTowerPartner(baseInfo);
            if(partner != null)
            {
                dual = true;
                altLabel = roleLabel == leftLabel ? rightLabel : leftLabel;
                altIndex = FindAssignmentIndex(patternId, altLabel);
            }
        }

        // Show every station of the active pattern dimmed. Own station is tethered (MyRole, below);
        // the alternate candidate (if any) is drawn brighter right after this loop.
        if(C.ShowAllRoles)
        {
            var pattern = Patterns[patternId];
            for(var i = 0; i < pattern.Assignments.Length && i < MaxPatternAssignments; i++)
            {
                if(i == assignmentIndex || (dual && i == altIndex))
                    continue;
                if(ResolvePositionRule(GetConfiguredRule(patternId, i)) is not { } otherPosition)
                    continue;
                EnableRoleMarker(GetRolePreviewElementName(i), otherPosition,
                    pattern.Assignments[i].Label, tether: false, color: OtherRoleColor);
            }
        }

        if(dual
            && altIndex >= 0
            && ResolvePositionRule(GetConfiguredRule(patternId, altIndex)) is { } altPosition)
        {
            EnableRoleMarker(ElMyRoleAlt, altPosition, altLabel, tether: false, color: CandidateAltColor);
            EnablePartnerMarker(partner!.Player.Position);
        }
        else
        {
            DisableDualMarkers();
    ...
    // Returns the left/right slot labels of a duplicate call-out pair when roleLabel belongs to one
    // (211 stacks, 022 spreads, 022 cones); false otherwise.
    private static bool TryGetDualCandidateRoleLabels(string roleLabel, out string leftLabel, out string rightLabel)
    {
        switch(roleLabel)
        {
            case "211_LeftTowerStack":
            case "211_RightTowerStack":
                leftLabel = "211_LeftTowerStack";
                rightLabel = "211_RightTowerStack";
                return true;
            case "022_RightTowerSpreadLeft":
            case "022_RightTowerSpreadRight":
                leftLabel = "022_RightTowerSpreadLeft";
                rightLabel = "022_RightTowerSpreadRight";
                return true;
            case "022_LeftTowerConeLeft":
            case "022_LeftTowerConeRight":
                leftLabel = "022_LeftTowerConeLeft";
                rightLabel = "022_LeftTowerConeRight";
                return true;
            default:
                leftLabel = "";
                rightLabel = "";
    ...
    // The other tower player sharing the same debuff (the duplicate call-out partner).
    ...
    private void EnablePartnerMarker(Vector3 playerPosition)
    {
        if(!Controller.TryGetElementByName(ElPartnerMark, out var element))
            return;

        element.SetRefPosition(playerPosition);
        element.color = PartnerMarkColor;
        element.overlayText = "Same call-out";
    ...
    private void DisableDualMarkers()
    {
        ...
        if(Controller.TryGetElementByName(ElPartnerMark, out var partner))
        {
            partner.Enabled = false;
            partner.tether = false;
        }
    }
    ...
    // Runs every frame while in P2. Captures each player's bearing from center at the moment their
    // debuff first appears (or changes kind) and freezes it; clears it when the debuff is gone.
    private void UpdateDebuffSnapshots()
    {
        if(!TryEnsureInfos())
            return;
        ...
        foreach(var info in _infos)
        {
            var current = GetDebuffKind(info.Player);
            if(current == DebuffKind.None)
            {
                info.SnapshotBearing = null;
                info.SnapshotBearingDebuff = DebuffKind.None;
            }
            else if(info.SnapshotBearing == null || info.SnapshotBearingDebuff != current)
            {
                info.SnapshotBearing = GetAngleFromTrueNorth(info.Player.Position);
                info.SnapshotBearingDebuff = current;
            }
    ...
    private string? ResolvePattern211(PlayerInfo info)
    {
        if(IsTower(info))
        {
            ...
            {
                DebuffKind.Stack => ResolveDuplicateSlot(
                    _infos.Where(i => IsTower(i) && i.Debuff == DebuffKind.Stack), info,
                    "211_LeftTowerStack", "211_RightTowerStack"),
    ...
    private string? ResolvePattern022(PlayerInfo info)
    {
        if(IsTower(info))
        {
            ...
            {
                return ResolveDuplicateSlot(
                    _infos.Where(i => IsTower(i) && i.Debuff == DebuffKind.Spread), info,
                    "022_RightTowerSpreadLeft", "022_RightTowerSpreadRight");
            }

            if(info.Debuff == DebuffKind.Cone)
            {
                return ResolveDuplicateSlot(
                    _infos.Where(i => IsTower(i) && i.Debuff == DebuffKind.Cone), info,
                    "022_LeftTowerConeLeft", "022_LeftTowerConeRight");
            }
    ...
    {
        var a = NormalizeAngle360(angle);
        return a > 180f ? a - 360f : a;
    }

    // Facing arena center, a player's left is clockwise (increasing compass bearing). Returns true if
    // 'target' is the relatively-left player versus 'partner' using the frozen snapshot bearings, or
    // null when either snapshot is missing or the two are too close to tell apart.
    private static bool? TryIsLeftBySnapshot(PlayerInfo target, PlayerInfo partner)
    {
        if(target.SnapshotBearing is not { } targetBearing || partner.SnapshotBearing is not { } partnerBearing)
            return null;

        var diff = NormalizeAngleSigned(targetBearing - partnerBearing);
        if(MathF.Abs(diff) < SnapshotMinSeparationDeg)
            return null;

        return diff > 0f;
    }

    // Splits a duplicate call-out subset (expected size 2) into left/right slots by snapshot bearing,
    // falling back to priority rank when the snapshot cannot decide.
    private string? ResolveDuplicateSlot(IEnumerable<PlayerInfo> subset, PlayerInfo target,
        string leftLabel, string rightLabel)
    {
        var list = subset.ToList();
        var partner = list.FirstOrDefault(i => i.Player.EntityId != target.Player.EntityId);
        if(partner != null && TryIsLeftBySnapshot(target, partner) is { } isLeft)
            return isLeft ? leftLabel : rightLabel;

        return GetPriorityRank(list, target) switch
        {
            0 => leftLabel,
            1 => rightLabel,
            _ => null,
    ...
    private bool TryComputeLiveRoles(out int patternId, out string roleLabel)
    {
        ...
        if(_step == Step1DisplayStep)
        {
            ...
            // Wave 1 is always the 211 stack wave (the initial group requires two stack pairs). Reuse
            // the relative-position resolution so its two stacks split left/right exactly like later
            // waves; the A/B half assignment from TryResolveInitialGroup is preserved.
            ResolvePattern(0);
